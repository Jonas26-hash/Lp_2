...
public class ProyectoServiceImpl implements ProyectoService {
    ...
    public ProyectoDTO actualizar(Long id, ProyectoDTO dto) {
        Proyecto p = repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Proyecto no encontrado"));
    ...
    public ProyectoDTO buscarPorId(Long id) {
        return repo.findById(id).map(ProyectoMapper::toDTO)
    ...
    public void eliminar(Long id) {
        if (!repo.existsById(id)) throw new ResourceNotFoundException("Proyecto no encontrado");
        repo.deleteById(id);
    }
}
